...
class Conv2DConditioner(Conditioner):
    ...
    def __init__(
        ...
        self.cnn = torch.nn.Sequential(
            ...
            torch.nn.Dropout(dropout),
            GatedConv2d(
                64, output_channels, kernel_size=last_kernel_size, stride=1, padding=0
            ),
        )
        if context_size > 0:
    ...
    def forward(self, inputs, **kwargs):
        if self.context_size > 0:  # some of the units are considered "context"
            inputs, context = torch.split(
                inputs, self.input_size - self.context_size, -1
            )
        else:
            context = None
        # [B, H*W] -> [B, 1, W, H]
        inputs = inputs.reshape(
            inputs.size(0), self.input_channels, self.height, self.width
        ).permute(0, 1, 3, 2)
        h = self.cnn(inputs)
    ...
    def __init__(
        ...
        if context_size > 0:
            self.input_layer = torch.nn.Sequential(
                torch.nn.Linear(input_size, input_size - context_size), torch.nn.ReLU()
            )
        else:
